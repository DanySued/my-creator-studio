    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for keyword in keywords:
            try:
                ...
                raise ValueError(f"Pexels search failed: {str(e)}")

    if not videos:
        raise ValueError(f"No videos found for keywords: {keywords}")

    # Extract video file URLs and durations
    video_list = []
    for video in videos:
        ...
        # Among candidates, pick highest resolution
        best_file = max(candidate_pool, key=lambda f: f.get("height", 0) * f.get("width", 0))
